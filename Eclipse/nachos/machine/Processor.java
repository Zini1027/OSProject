 ...
 *
 * <p>
 * The <tt>Processor</tt> API is re-entrant, so a single simulated processor can be shared by
 * multiple user threads.
 *
 * <p>
 * An instance of a <tt>Processor</tt> also includes pages of physical memory accessible to user
...
public final class Processor {
     ...
     *
     * <p>
     * When the exception handler is called, interrupts will be enabled, and the CPU cause register
     ...
     *
     * <p>
     * If <tt>true</tt>, this processor has a software-managed TLB; use <tt>getTLBSize()</tt>,
     * <tt>readTLBEntry()</tt>, and <tt>writeTLBEntry()</tt>.
     *
     * <p>
     * Using a method associated with the wrong address translation mechanism will result in an
     * assertion failure.
     *
    ...
    /**
     * Set the page table pointer. All further address translations will use the specified page
     * table. The size of the current address space will be determined from the length of the page
     * table array.
     *
     * @param pageTable
     ...
     *
     * <p>
     * The TLB is fully associative, so the location of an entry within the TLB does not affect
     * anything.
     *
     * @param number
     *            the index into the TLB.
     ...
     * @param offset
     *            the offset within the page. Must be between <tt>0</tt> and <tt>pageSize - 1</tt>.
    ...
    /**
     * Translate a virtual address into a physical address, using either a page table or a TLB.
     * Check for alignment, make sure the virtual page is valid, make sure a read-only page is not
     * being written, make sure the resulting physical page is valid, and then return the resulting
     * physical address.
     *
     * @param vaddr
    ...
    /**
     * Read </i>size</i> (1, 2, or 4) bytes of virtual memory at <i>vaddr</i>, and return the
     * result.
     *
     * @param vaddr
     ...
     * @param nextLoadMask
     *            the mask specifying which bits in the new target are to be overwritten. If a bit
     *            in <tt>nextLoadMask</tt> is 0, then the corresponding bit of register
     ...
     *
     * <p>
     * Transfer the contents of the nextPC register into the PC register, and then add 4 to the
     * value in the nextPC register. Same as <tt>advancePC(readRegister(regNextPC)+4)</tt>.
     *
     * <p>
     * Use after handling a syscall exception so that the processor will move on to the next
     * instruction.
     */
